use crate::prelude::*;
use nalgebra::{Matrix3, UnitQuaternion, UnitVector3, Vector3};
use strum_macros::EnumString;

pub(crate) type Vector = Vector3<f64>;
pub(crate) type Vectors = Vec<Vector>;
pub(crate) type NearestNeighborDist = Vec<(usize, f64)>;
pub(crate) type UVector = UnitVector3<f64>;
pub(crate) type UQuaternion = UnitQuaternion<f64>;
pub(crate) type Matrix = Matrix3<f64>;

pub(crate) trait AsVector {
    fn as_vector(&self) -> &Vector;
    fn as_vector_mut(&mut self) -> &mut Vector;
}

pub(crate) trait AsMatrix {
    fn as_matrix(&self) -> &Matrix;
    fn as_matrix_mut(&mut self) -> &mut Matrix;
}

#[derive(Debug, PartialEq, EnumString, Clone, Copy)]
pub(crate) enum Element {
    Sr,
    Ti,
    O,
}

impl Element {
    #[must_use]
    pub(crate) fn get_mass(self) -> f64 {
        match self {
            Element::Sr => 87.62,
            Element::Ti => 47.867,
            Element::O => 15.999,
        }
    }
}

pub(crate) type Elements = Vec<Element>;

#[derive(Debug, EnumString, PartialEq, Clone, Copy)]
pub(crate) enum Coordinates {
    #[strum(serialize = "D", serialize = "Direct")]
    Direct,
    #[strum(serialize = "C", serialize = "Cartesian")]
    Cartesian
}

#[derive(Clone, Copy, Debug, PartialEq)]
pub(crate) enum DWType {
    HT,
    HH,
    Apb
}

#[derive(Debug, Clone, Copy)]
pub(crate) enum DWSide {
    Left,
    Right,
}

#[derive(Clone, Copy, Debug)]
pub(crate) enum PhaseFactor {
    Negative,
    Positive
}

impl PhaseFactor {
    pub(crate) fn to_int(self) -> i32 {
        match self {
            Self::Negative => -1,
            Self::Positive => 1
        }
    }
}

#[derive(Clone, Copy, PartialEq, Debug)]
pub(crate) struct CellAlignment {
    pub(crate) unit_quaternion: UQuaternion,
    pub(crate) ref_variant: RotationVariant, 
    pub(crate) error: f64
}

#[derive(PartialEq, Debug, Copy, Clone, Default)]
pub(crate) struct Atom {
    pub(crate) atom_type: Option<Element>,
    pub(crate) position: Vector,
}

impl Atom {
    #[must_use]
    pub(crate)fn new(atom_type: Element, position: Vector) -> Self {
        Atom { atom_type: Some(atom_type), position }
    }
}

pub(crate)type Atoms = Vec<Atom>;
pub(crate)type AtomPair = (Atom, Atom);
pub(crate)type AtomPairs = Vec<AtomPair>;

pub(crate)struct SupercellAtoms {
    pub(crate)a: Atoms,
    pub(crate)b: Atoms,
    pub(crate)x: Atoms
}

impl SupercellAtoms {
    pub(crate)fn new( positions: &Vectors, n_sr: usize, n_ti: usize, n_o: usize) -> Result<Self> {
        let n_elements = n_sr + n_ti + n_o;

        if positions.len() == n_elements {
            let a_site = positions[..n_sr].iter().map(|pos| Atom{ atom_type: Some(Element::Sr), position: *pos }).collect();
            let b_site = positions[n_sr..n_sr+n_ti].iter().map(|pos| Atom{ atom_type: Some(Element::Ti), position: *pos }).collect();
            let c_site = positions[n_sr+n_ti..].iter().map(|pos| Atom{ atom_type: Some(Element::O), position: *pos }).collect();

            Ok(SupercellAtoms { a: a_site, b: b_site, x: c_site })
        } else {
            Err(Error::SizeMismatch { expected: n_elements, got: positions.len() })
        }
    }
}

pub(crate)struct AtomDisplacements {
    pub(crate) a: Vectors,
    pub(crate) b: Vectors,
    pub(crate) x: Vectors
}

#[derive(Debug, Clone, Copy, PartialEq)]
pub(crate)enum RotationVariant {
    R0,
    R90,
    R180,
    R270
}

#[derive(PartialEq, Debug, Clone)]
pub(crate)struct ReadResult {
    pub(crate)positions: Vectors,
    pub(crate)element_count: Vec<(Element, usize)>,
    pub(crate)cell_matrix: Matrix,
    pub(crate)coordinate_type: Coordinates,
}

#[derive(PartialEq, Debug)]
pub(crate)enum ReadResultVariant {
    ReadResult { res: ReadResult },
    ReadResults { res: Vec<ReadResult> }
}

#[cfg(test)]
mod tests {
    use super::*;

    #[test]
    fn test_supercell_atoms_new_succeed() {
        let sr = Vector::new(1.0, 1.0, 1.0);
        let ti = Vector::new(2.0, 2.0, 2.0);
        let o1 = Vector::new(3.0, 3.0, 3.0);
        let o2 = Vector::new(4.0, 4.0, 4.0);
        let o3 = Vector::new(5.0, 5.0, 5.0);

        let positions = vec![sr, ti, o1, o2, o3];

        let supercell_atoms = SupercellAtoms::new(&positions, 1, 1, 3).unwrap();

        assert_eq!(supercell_atoms.a[0].atom_type.unwrap(), Element::Sr);
        assert_eq!(supercell_atoms.a[0].position, sr);

        assert_eq!(supercell_atoms.b[0].atom_type.unwrap(), Element::Ti);
        assert_eq!(supercell_atoms.b[0].position, ti);

        supercell_atoms.x.iter().zip(vec![o1, o2, o3]).for_each(|(atom, expected)| {
            assert_eq!(atom.atom_type.unwrap(), Element::O);
            assert_eq!(atom.position, expected);
        });

        assert_eq!(supercell_atoms.a.len(), 1);
        assert_eq!(supercell_atoms.b.len(), 1);
        assert_eq!(supercell_atoms.x.len(), 3);
    }

    #[test]
    fn test_supercell_atoms_new_fail() {
        let sr = Vector::new(1.0, 1.0, 1.0);
        let ti = Vector::new(2.0, 2.0, 2.0);
        let o1 = Vector::new(3.0, 3.0, 3.0);
        let o2 = Vector::new(4.0, 4.0, 4.0);
        let o3 = Vector::new(5.0, 5.0, 5.0);

        let positions = vec![sr, ti, o1, o2, o3];

        assert!(SupercellAtoms::new(&positions, 2, 1, 3).is_err())
    }
}
