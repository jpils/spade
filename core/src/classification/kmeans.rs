...
pub(crate)struct Cluster {
    pub(crate)bb_descriptors: BbDescriptors,
    pub(crate)centroid: Descriptor
}

pub(crate)type Clusters = Vec<Cluster>;

pub(crate)fn k_means(data: &BbDescriptors, n_clusters: usize) -> Clusters {
...
pub(crate)fn get_cluster_center(cluster: &Descriptors) -> Descriptor {
...
fn converged(current: &[Descriptor], new: &[Descriptor]) -> bool {
    current
        .iter()
        .zip(new)
        .all(|(o, n)| (o - n)
