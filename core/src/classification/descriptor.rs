...
pub(crate)struct BbDescriptor {
    pub(crate)descriptor: Descriptor,
    pub(crate)reference_b_atom: Atom
}

pub(crate) type BbDescriptors = Vec<BbDescriptor>;

pub(crate) fn calculate_bb_descriptors(mut b_b: Vec<NearestNeighbor>, b_x: &[NearestNeighbor]) -> BbDescriptors {
    ...
    while let Some(b) = b_b.pop() {
        ...
        let current_desc = get_b_site_descriptor(current_x);
        let neighbor_desc = get_b_site_descriptor(neighbor_x);
...
mod tests {
    ...
    fn test_calculate_bb_link_descriptors_averaging() {
        ...
        let b_b_pair = vec![
            NearestNeighbor {
                reference: atom_b1,
                neighbors: vec![(atom_b2, atom_b2, 1.0)],
            },
            NearestNeighbor {
                reference: atom_b2,
                neighbors: vec![(atom_b1, atom_b1, 1.0)],
            }
        ];
        ...
        for desc in results {
            if desc.reference_b_atom.position == b1_center || 
                desc.reference_b_atom.position == b2_center  {
