...
#[derive(Clone, Debug)]
pub(crate)struct SimCell(pub(crate) UnitCell);
...
impl SimCellBuilder {
    ...
    pub(crate)fn atoms(&mut self, atoms: Atoms) -> &mut Self {
        self.atoms = Some(atoms);
        self
    }

    pub(crate)fn cell_matrix(&mut self, cell_matrix: Matrix) -> &mut Self {
        self.cell_matrix = Some(cell_matrix);
        self
    }

    pub(crate)fn dw_type(&mut self, dw_type: DWType) -> &mut Self {
        self.dw_type = Some(dw_type);
        self
    }

    /// outputs unit cell with atom positions in cartesian coordinates
    pub(crate)fn build(&self) -> Result<SimCell> {
...
mod tests {
    ...
    fn atoms() -> Atoms {
        let a = 1.0;
        let mut atoms = vec![
            Atom { atom_type: Some(Element::Ti), position: Vector::zeros()},
