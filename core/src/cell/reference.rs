...
pub(crate)struct RefCellBuilder {
...
impl RefCellBuilder {
    ...
    pub(crate)fn geometry(&mut self, geometry: CellGeometry) -> &mut Self {
        self.geometry = Some(geometry);
        self
    }

    pub(crate)fn lattice_constant(&mut self, lattice_constant: f64) -> &mut Self {
        self.lattice_constant = Some(lattice_constant);
        self
    }

    pub(crate)fn tilt_angle(&mut self, tilt_angle: f64 /* rad */) -> &mut Self {
        self.tilt_angle = Some(tilt_angle);
        self
    }

    pub(crate)fn tilt_axis(&mut self, tilt_axis: UVector) -> &mut Self {
        self.tilt_axis = Some(tilt_axis);
        self
    }

    pub(crate)fn elements(&mut self, elements: Elements) -> &mut Self {
        self.elements = Some(elements);
        self
    }

    pub(crate)fn rotation_variant(&mut self, rotation_variant: RotationVariant) -> &mut Self {
        self.rotation_variant = Some(rotation_variant);
        self
    }

    pub(crate)fn build(&self) -> Result<RefCell> {
...
impl RefCellBuilder {
    fn validate_fields(&self) -> Result<BuildContext<'_>> {
        ...
        let tilt_angle = self.tilt_angle.unwrap_or(0.0);
...
fn build_cubic(ctx: BuildContext) -> RefCell {
...
fn get_cubic_ref_atoms(
    ...
    UnitCellAtoms { a: a_site_atoms, b: b_site_atoms, x: x_site_atoms }
}
